...
public class ExcelExtractor implements Extractor.ContentExtractor {
    ...
    public void run(
        ...
        try {
            final IRI documentIRI = context.getDocumentURI();
    ...
    private IRI getSheetIRI(IRI documentIRI, Sheet sheet) {
        return RDFUtils.uri( documentIRI.toString() + "/sheet/" + sheet.getSheetName() );
    }

    private IRI getRowIRI(IRI sheetIRI, Row row) {
        return  RDFUtils.uri( sheetIRI.toString() + "/" + row.getRowNum() );
    }
